...
#                raise ValueError("Error executing command '[{}]> {}'.\t {}".format(host,cmd,stderr_str))
            ...
            # Compile the code
            cmd = "mkdir -p AntibIoTic/bin && gcc AntibIoTic/src/*.c -o AntibIoTic/bin/antibiotic_pi_debug -std=c99 -pthread -g -DDEBUG"
            print("[{}] > {}".format(host,cmd))
            stdin, stdout, stderr = client.exec_command(cmd)
            return_code = stdout.channel.recv_exit_status()
            stderr_str = stderr.read().decode('utf-8')
            ...
            if args.run:
                cmd = "chmod +x AntibIoTic/bin/antibiotic_pi_debug && AntibIoTic/bin/antibiotic_pi_debug "+gateway+" 3"
                print("[{}] > {}".format(host,cmd))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--gateway-ip", default="192.170.0.2", help="AntibIoTic gateway IP address")
